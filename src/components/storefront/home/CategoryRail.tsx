...
export function CategoryRail({ categories }: { categories: Category[] }) {
  return (
    ...
    <div className="no-scrollbar overflow-x-auto overscroll-x-contain px-5">
      <div className="grid w-max grid-flow-col grid-rows-2 gap-x-4 gap-y-5">
      {categories.map((category) => {
        ...
        return (
          <Link
            ...
            className="group flex w-[4.75rem] flex-col items-center gap-2 text-center"
          >
            <span className="relative size-[4.75rem] overflow-hidden rounded-xl bg-sunk ring-1 ring-line-hair transition-transform duration-200 ease-out-quart group-active:scale-95">
              {photo && (
                <Image
                  src={photo}
                  alt=""
